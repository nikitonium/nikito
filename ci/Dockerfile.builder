...
# nikito_hash
RUN git clone https://github.com/nikitonium/nikito_hash
RUN cd nikito_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} nikito
RUN useradd -u ${USER_ID} -g nikito -s /bin/bash -m -d /nikito nikito
...
RUN mkdir /nikito-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /nikito-src && \
