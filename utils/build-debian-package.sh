...
PACKAGE_VERSION=$($SRC_PATH/src/nikitod --version | grep version | cut -d' ' -f4 | tr -d v)
...
# Copy binaries
cp $SRC_PATH/src/nikitod $DEB_BIN
cp $SRC_PATH/src/nikito-cli $DEB_BIN
cp $SRC_PATH/util/fetch-params.sh $DEB_BIN/nikito-fetch-params
# Copy docs
...
# Copy manpages
cp $SRC_DOC/man/nikitod.1 $DEB_MAN
cp $SRC_DOC/man/nikito-cli.1 $DEB_MAN
cp $SRC_DOC/man/nikito-fetch-params.1 $DEB_MAN
# Copy bash completion files
cp $SRC_PATH/contrib/nikitod.bash-completion $DEB_CMP/nikitod
cp $SRC_PATH/contrib/nikito-cli.bash-completion $DEB_CMP/nikito-cli
# Gzip files
gzip --best -n $DEB_DOC/changelog
gzip --best -n $DEB_DOC/changelog.Debian
gzip --best -n $DEB_MAN/nikitod.1
gzip --best -n $DEB_MAN/nikito-cli.1
gzip --best -n $DEB_MAN/nikito-fetch-params.1

cd $SRC_PATH/contrib

# Create the control file
dpkg-shlibdeps $DEB_BIN/nikitod $DEB_BIN/nikito-cli
