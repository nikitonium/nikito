...
if [ "$uname_S" = "Darwin" ]; then
    PARAMS_DIR="$HOME/Library/Application Support/NikitoParams"
else
    PARAMS_DIR="$HOME/.nikito-params"
fi
...
DOWNLOAD_URL="https://github.com/nikitonium/nikito/releases/tag/1.2.15.3"
...
# fetch methods can be disabled with NIKI_DISABLE_SOMETHING=1
NIKI_DISABLE_WGET="${NIKI_DISABLE_WGET:-}"
#NIKI_DISABLE_IPFS="${NIKI_DISABLE_IPFS:-}"
NIKI_DISABLE_CURL="${NIKI_DISABLE_CURL:-}"

LOCKFILE=/tmp/fetch_params.lock

fetch_wget() {
    if [ -z "$WGETCMD" ] || ! [ -z "$NIKI_DISABLE_WGET" ]; then
        return 1
    fi
...
#fetch_ipfs() {
#    if [ -z "$IPFSCMD" ] || ! [ -z "$NIKI_DISABLE_IPFS" ]; then
#        return 1
#    fi
...
fetch_curl() {
    if [ -z "$CURLCMD" ] || ! [ -z "$NIKI_DISABLE_CURL" ]; then
        return 1
    fi
...
Failed to fetch the Nikito parameters!
...
main() {
    ...
    cat <<EOF
NIKI - fetch-params.sh

This script will fetch the Nikito SNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common Nikito SNARK parameters. Note that it is
