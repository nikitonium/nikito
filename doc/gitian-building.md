...
*Setup instructions for a Gitian build of Nikito Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Nikito
...
- [Building Nikito Core](#building-nikito-core)
...
Clone the git repositories for Nikito Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/nikitonium/nikito
...
in the Nikito Core repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Nikito Core
----------------

To build Nikito Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Nikito Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/nikito/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/nikitonium/nikito
...
For example:
```bash
URL=https://github.com/crowning-/nikito.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit nikito=${COMMIT} --url nikito=${URL} ../nikito/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit nikito=${COMMIT} --url nikito=${URL} ../nikito/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit nikito=${COMMIT} --url nikito=${URL} ../nikito/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the nikito git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=bionic on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../nikito/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/nikito
SIGPATH=/some/root/path/nikito-detached-sigs

./bin/gbuild --url nikito=${BTCPATH},signature=${SIGPATH} ../nikito/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/nikito-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/nikito-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/nikito-osx-build.assert
```
...
[nikito/gitian.sigs](https://github.com/nikitonium/gitian.sigs/) repository, or if that's not possible to create a pull
request.
