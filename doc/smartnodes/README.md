...
Add a system user to run nikitod:

```bash
adduser <username_here>
```

Get Nikito wallet / daemon and bootstrap:

```bash
wget https://github.com/nikitonium/nikito/releases/download/v2.0.2.1/ltrm-ubuntu-2.0.2.1.zip
unzip ltrm-ubuntu-2.0.2.1.zip
chmod -R 755 ltrm-ubuntu-2.0.2.1
mkdir ~/.nikitocore && touch ~/.nikitocore/nikito.conf
echo '
rpcuser=username
...
txindex=1
daemon=1
' >> ~/.nikitocore/nikito.conf
wget -P /.nikitocore https://github.com/nikitonium/Nikito/releases/download/v2.0.2.1/bootstrap.zip && unzip /.nikitocore/bootstrap.zip -d /.nikitocore
wget -P /.nikitocore https://github.com/nikitonium/Nikito/releases/download/v2.0.2.1/powcache.dat
~/ltrm-ubuntu-2.0.2.1/./nikitod -daemon
```
```bash
#Common commands:
~/ltrm-ubuntu-2.0.2.1/./nikito-cli getblockchaininfo
~/ltrm-ubuntu-2.0.2.1/./nikito-cli getnetworkinfo
~/ltrm-ubuntu-2.0.2.1/./nikito-cli getbalance
~/ltrm-ubuntu-2.0.2.1/./nikito-cli stop
```

You should see Nikitod server starting. You can confirm it is running <code>top -c</code> it will be using 100% CPU (1 core). This shows you it is working, we will return to it later.
...
- Send 100,000 NIKI to self (this is the minimum collateral amount), Wait for 2 confirmations
- Smartnodes rules:
```
...
- <code>Fee address</code>: This is any address in your wallet which contains
  enough NIKI to pay the fee (cannot be the address which you sent the 1 million
  NIKI to). When you enter the protx quick_setup command it is a transaction and
  needs to be paid for. It is a very small amount 1/2 an NIKI is more than
...
When done with this, run nikitod from the terminal:

```bash
~/./nikitod -daemon
```

When this is done, wait a couple minutes until running `~/./nikito-cli smartnode status` to check the status of your smartnode. This should return: `Ready Ready`
