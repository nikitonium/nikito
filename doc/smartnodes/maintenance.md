...
- nikitod is unavailable or not functioning correctly (hung).
- Resource shortages (cpu, ram).
- Unstable network or outage.

You can check if your Smartnode is banned directly in the QT wallet in the Smartnodes tab, check the "My Smartnodes Only". Or by going to the nikito explorer > Smartnodes Tab > search for your Smartnode IP address.

### Resolving

1. Check if nikitod is responsive;

```bash
./nikito-cli getblockchaininfo
```
...
If the command from step 1 is very slow to respond or does not respond, nikitod is in a hung state and needs to be killed. Kill it with:

```bash
ps aux  |  grep -i nikitod  |  awk '{print $2}'  |  xargs sudo kill -15
```

2. If you see "No such process" kill was successful, if you see a process listed you will need to issue the command again to change -15 to -9 which is a harsher termination. Restart nikitod:
...
```bash
tail -f ~/.nikitocore/debug.log # Ctrl + C to exit tail.
```

An alternative way to check and see when nikitod is done syncing is to use the "top" command. While syncing nikitod will consistently use nearly 100% of a core. When that use drops substantially it is done syncing. Depending on the nodes PoSe score it can take anywhere from 2 to 6 hours for PoSe ban to be lifted. To speed that process you can issue a "protx update_service" command and the ban will be lifted in 1-2 blocks.
...
- Third is the Smartnodes BLS private key. This is stored on your Smartnode in ~/.nikitocore/nikito.conf and you can retrieve it easily by doing:

```bash
cat ~/.nikitocore/nikito.conf | grep bls
```
...
3. Resolving resource related PoSe ban issues. Make sure nikitod always has access to at least 2 full CPU cores. We strongly recommend against mining on your Smartnodes. Mining on them will endup in performance issues and PoSe bans. If you find nikitod is sometimes not running this is often due to it being killed by OOM (Out Of Memory Killer) this is a process of the Linux kernel that will kill certain processes when the system is critically low on memory. Search your system logs to help diagnose the issue, as an example on ubuntu 18:
...
```bash
#! /bin/bash
ps -ef | grep "nikitod" | grep -v grep
if [ $? -ne 0 ]
then
echo "$(date)" >> /log-nikitod.txt
cd /nikito && ./nikitod -daemon # Modify the directory name to your own
else
echo "running"
fi
```
...
As the nikito network matures Smartnode collateral ramps up and you will need to recreate your Smartnode in order to increase the collateral amount. 
- Smartnodes rules:
```
        0 - 20000     100,000 NIKI
    20001 - 100000    200,000 NIKI
   100001 - 200000    400,000 NIKI
   200001 - 300000    600,000 NIKI
   300001 - 400000    800,000 NIKI
   400001 - forever   1,000,000 NIKI
```
...
Collateral fee address can be any address in your wallet that has funds to pay for the transaction, the transaction fee is small as an example I just made one transaction and the cost was 0.00000525 NIKI. To see which of your addresses have funds use "listaddressbalances” in debug console or nikito-cli.

:::

**protx quick_setup command example:**
```bash
./nikito-cli protx quick_setup "c4bbcde9771668fa640c263d4b964f688b0f039f7b684e715d92e4012369fea6" "1" "127.0.0.1:21001" "LFbWv94ZfueciwVVpHLMdqFayaXAS4sBxP"
```

So structure is: `transaction ID | collateral index | smartnode IP | port | collateral fee address`

Using `QT > Debug Console` just remove "./nikito-cli" from the above command. Fire the command and if it is successful you will see a bunch of output, the command has created the needed nikito.conf file for your Smartnode. Check the last line of the output as it will tell you the location of the .conf file, notice that it is named differently for example: `RMsneH79j9gioE1jFsuxTFUxaZ1JSp53SN_nikito.conf`

That is your NIKI collateral address that you created at the start of this guide and is so the .conf file is easy to identify and link to corresponding smartnode. As this is a collateral upgrade all we need from the generated .conf file is the `smartnodeblsprivkey`.
...
```bash
nano ~/.nikitocore/nikito.conf
```

Swap out the old bls private key with the new, exit, and start nikitod:
