...
const char * const BITCOIN_CONF_FILENAME = "nikito.conf";
const char * const BITCOIN_PID_FILENAME = "nikitod.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "nikitod -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\NikitoCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\NikitoCore
    // Mac: ~/Library/Application Support/NikitoCore
    // Unix: ~/.nikitocore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "NikitoCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/NikitoCore";
#else
    // Unix
    return pathRet / ".nikitocore";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty nikito.conf if it does not excist
...
std::string CopyrightHolders(const std::string& strPrefix, unsigned int nStartYear, unsigned int nEndYear)
{
    ...
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Nikito Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + strprintf(" %u-%u ", 2021, nEndYear) + "The Nikito Core developers";
    }
